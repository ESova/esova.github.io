...
function PromocodeComponent (db, engine) { // SIGNUP COMPONENT - OVERLAY
  ...
  function AUTHED (error, help) {
    if (error) console.error('AUTH FAILED')
      ...
      if (this === 'newuser') {
        var url = 'http://45.55.222.16/last_lesson'
        var id = JSON.parse(creds).id
        minixhr({
          url: url+'?id='+id+'&last_lesson=4',
          method: 'POST'
        })
      }
      api('getlessonNumber', function (error, number) { })
    }
  }
}
