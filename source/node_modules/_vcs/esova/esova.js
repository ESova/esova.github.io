  ...
  db.get('esova-token', function (error, TOKEN) {
    api(action, function (error, data) {
      ...
      if ((error||{}).status === 401) alert("log in again")
        ...
        var CURRENT = String(0) // curren lesson
        ...
        var lessons = data.filter(function(lesson) {
          if (lesson.type === 'dir') {
            ...
            for(var pad=''; 3-digits; (pad+='0',digits++)){}
            var number = lesson.name.substring(lesson.name.length-3, lesson.name.length)
            var shouldDownload = number === pad + CURRENT
            return shouldDownload ? true : false
          } else return false
        })
        var no = lessons.length
        lessons.forEach(function (lesson) {
          var url = lesson.git_url
          minixhr({
            url: url+'?access_token='+TOKEN
          }, function (data, response, xhr, header) {
            LESSONS.push(data)
            no--
            if(!no) gogo(LESSONS)
          })
        })
        function gogo (LESSONS) {
          LESSONS.forEach(function (lesson) {
            lesson = JSON.parse(lesson)
            var len = lesson.tree.length
            lesson.tree.forEach(function (file) {
              minixhr({
                url: file.url+'?access_token='+TOKEN
              }, function (data, response, xhr, header) {
        ...
        function final (FILES) {
          ...
          //   if ()
          // })
          var md = JSON.parse(FILES['description.rs.md']).content
          var markdown = b64_to_utf8(md).split('\n')
          var name = markdown.shift()
          var title = name.split('# ')[1]
          var text = markdown.join('\n')

          debugger
          // @TODO: name, title & text
          // 1. store in db
          // 2. render into page

          // @TODO: set by parent through subEngine
          // getSkillboxVtree()
          // engine.push( { name: 'skillbox', vtree: vtree })
