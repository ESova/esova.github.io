...
function header (db, engine) {
  ...
  function renderHeader () {
    var vtree = h('div', { className: x('header') }, [
      h('div', { className: x('menubar') }, [
        ...
        (CREDS||{}).id ? loggedIn() : loggedOut('Mrs. eSova', owl)
      ])
    ])
    // PUSH UPDATES
    engine.push({
      ...
      return h('div', { className: x('usernav') }, [
        ...
        h('div', {
          ...
          h('div', { className: x('avatar-name') }, [
            USER.name ? USER.name : LOCALE["text/profile:anon"]
          ]),
  ...
  db.on('put', function (key, value) {
    if (key === 'esova-credentials') { // LISTEN TO
      ...
      if (CREDS.id !== creds.id) {
        CREDS.id = creds.id // UPDATE CACHE
  ...
  var LOCALE = {
    "text/home:anon": "Lessons",
    "text/home:user": "My Lesson",
    "text/messages": "Messages",
    "text/profile:anon": "Sign In"
  }
  ...
  function onsignin (event) {
    ...
    db.get('esova-credentials', function (error, credentials) {
      ...
      if (creds.id) auth(db, function noop (error, help) { })
      else {
        db.get('esova-currentLesson', function (error, lastlesson) {
          if (lastlesson <= FREE_LESSONS) creds.status = 'not ready'
          else creds.status = 'ok'
          db.put('OVERLAY', JSON.stringify(creds))
        })
      }
    })
  }
  function onlogout (event) {
    ...
    db.put('esova-credentials', creds, function () {
      debugger
