...
function skillbox (db, engine) {
  ...
  var STATUS = {
    ...
    navright: undefined,
    // LOCAL DATA STATUS
    // @TODO: switch chat url
    chaturl: 'https://gitter.im/ESova/eSovaITinfluencers/~embed',
    // chaturl: 'https://gitter.im/ESova/program/~embed'
    id: undefined,
  ...
  function renderSkillbox (/*error,*/ update) {
    ...
    // RENDER
    var isAllowed = +STATUS.currentlesson <= FREE_LESSONS ||
      STATUS.id && (STATUS.currentlesson <= STATUS.lastlesson ||
        STATUS.currentlesson <= 5)
            ...
            h('button', {
              ...
              onclick: function (event) {
                db.get('esova-currentLesson', function (error, lastlesson) {
                  var X
                  if (lastlesson <= FREE_LESSONS) X = { status: 'not ready' }
                  else X = { status: 'ok' }
                  db.put('OVERLAY', JSON.stringify(X))
                })
              },
  ...
  function updateLessonContent () {
    api('getlessondescription', function (error, lesson) {
      ...
      else db.put('esova-lessonDescription', JSON.stringify(lesson), function (error) {
