...
function skillbox (db, engine) {
  ...
  function renderSkillbox (/*error,*/ update) {
    ...
    var hasNavleft = STATUS.currentlesson === '0'
    var hasNavright = STATUS.currentlesson >= STATUS.maxlesson

    var vtree = h('div', { className: x('skillbox') }, [
      h('div', { className: x('skillbox-nav') }, [
        hasNavleft ? h('span',{className:x('navleft-placeholder')}) : h('a', {
          ...
          onclick: function (event) {
            ...
            db.put('esova-lesson',(--STATUS.currentlesson)+'',function(error){
        ...
        hasNavright ? h('span',{className:x('navright-placeholder')}) : h('a', {
        ...
        h('div', { className: x('lesson') }, [
          h('div', { className: x('lesson-header') }, [
            ...
            h('a', {
              ...
              onclick: isAllowed ? function startlesson () {
                alert('allowed')
                // => start lesson
                debugger
              } : function notify () {
  ...
  function updateLessonContent () {
    ...
    api('getlesson', function (error, lesson) {
      // @TODO: what if a lesson is being requests that doesnt
      // exist on github?
      // => max-lesson should be set in that case
      if ((error||{}).status === 401) alert("log in again")
      else if (error) db.put('ERROR', 'api getlesson 401 access denied', ER)
