  ...
  if (!REGISTERED) { // @HACK - why? what to do? wtf? :-)
    ...
    db.on('put', function (key, value) {
      ...
      if (key === 'target') {
        ...
        console.log(value)
        console.log('=====ERROR ORIGIN=====')
      } else if (key === 'esova-credentials') {
        debugger
        if (value && typeof value.status === 'string' && value.status!=='ok') {
          console.log('AUTH FAIL :-/')
          if (value.status === 'no valid promocode') {


          // @TODO: click sign-in TRIGGERS this
            db.put('OVERLAY', true) // triggers re-render


          } else if (value.status === 'no valid token') {
            render(value)
          } else if (value.status === 'could not store credentials') {
            render(value)
          } else if (value.status === 'couldnt get "auth code" from github') {
            render(value)
          }
        } else {
...
function mainpage (db, engine) {
  ...
  function renderMainPage (/*error,*/ update) {
    ...
    var vtree = h('div', { className: x('app') }, [
      ...
      h('div', { className: x('esova', STATUS.inactive) }, [
  ...
  db.on('put', function (key, value) { // SET by auth.js
    if (key === 'OVERLAY') { // former: 'ui/promocode'
        debugger
      // replace by "get credentials" and "check status"
      // @TODO:
      // a certain STATUS can affect multiple components
      // => 1. an overlay showing/revealing itself
      // => 2. an existing component changing it's appearance (e.g. blurring)
      STATUS.inactive = value ? x('esova--inactive') : undefined
      // @TODO: comonents return nothing or a noop
      // @TODO: components render stuff when they are ready and need to using
      //        engine.push(vtree)
      // @TODO: components are only initialized once and need to
      //        unplug/dissolve (unload from memory) when not longer necessary
    }
  })
