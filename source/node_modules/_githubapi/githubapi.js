...
}(murl('https://api.github.com/repos/ESova/program/contents/{realkey}')))
/*****************************************************************************/
var CONTEXT = {
  'getlesson': { // petaction
    PETBASE: 'data/lesson/',
    REALBASE: 'repos/ESova/program/contents',
    realkey2petkey: function realkey2petkey (realkey, next) { // realapi2petapi
      next(null, this.PETBASE+(+realkey.split(this.REALBASE)[1]))
    },
    petkey2realkey: function petkey2realkey (petkey, next) { // petapi2realapi
      next(null, this.REALBASE+padleft(petkey.split(this.PETBASE)[1], 3, '0'))
    },
    // ??? translator === petAction2realAction ???
    translate: getLessonKey, // getLessonKey(db, next/*(error, lessonKey)*/)
    getPetKey: function getPet (key, next) { next(null, this.REALBASE+key) },
    getRealKey: function getReal (key, next) { next(null, this.PETBASE+key) },
...
function getLessonKey (db, next/*(error, lessonKey)*/) {
  ...
  db.get('esova-token', function (error, token) {
    if (error) db.get('esova-currentLesson', function (error, lesson) {
      ...
      if (error) esova(realkey, {token:token}, function (error, request) {
        if (error) next(new Error('lesson request failed'))
          ...
          return function save (data/*, response, xhr, headers*/) {
            ...
            else var db.put('esova-currentLesson', data, function (error) {
...
function execute (db, petaction, next/*(error, result)*/) {
  ...
  if(isInvalid(CON)) next(new Error('no valid pet action given'))
    ...
    if (error) next(error)
    else CON.getReal(lessonKey, function (error, realkey) {
      if (error) next(error)
      else CON.getPet(lessonKey, function (error, petkey) {
        if (error) next(error)
        else CON.action(db, petkey, realkey, function (error, lesson) {
          if (error) next(error)
          else next(null, lesson)
        })
      })
