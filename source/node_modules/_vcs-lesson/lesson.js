...
function LessonComponent (db, engine) {
      ...
      h('div', { className: x('video-container') }, [
        ...
        video(STATUS.video, { protocol: 'http:' }, function (video) {
          ...
          video.bind("end", function() {
            console.log(STATUS.slides)
            console.log(STATUS.slides.length)
  ...
  function renderQuestion (slide, lesson, render, next) {
    var vtree = h('div', { className: x('videopopup-question')}, [
      ...
      h('div', { className: x('submit')}, ['Potvrdite odgovor'] )
    ])
    render(vtree)
  }
      ...
      h('div', {
        ...
        onclick: function (event) {
          if (lesson === '0') {
            ...
            window.open(slide.link.url, '_blank')
            renderConfirm(slide, lesson, render, next)
          }
        }
      }, ['Kliknite ovaj link'] ),
      h('div', { className: x('smallprint')}, ['i nastavite na sledeći korak'] )
    ])
    render(vtree)
  }
  ...
  function renderBravo (slide, lesson, render, next) {
    STATUS.solved += 1
    if (lesson === '0') window.onfocus = function () {
      doBravo()
      window.focus = undefined
    }
    else doBravo()
    function doBravo () {
      var id = setTimeout(next, 2000)
      var vtree = h('div', {
        className: x('bravo-notification'),
        onclick: function (event) {
  ...
  function renderConfirm (slide, lesson, render, next) {
    var vtree = h('div', { className: x('confirm-dialog') }, [
      'CONFIRM DIALOG'
    ])
    render(vtree)
  }
  function renderSolution (slide, lesson, render, next) {
    var vtree = h('div', { className: x('videopopup-solution')}, [
      h('div', { className: x('exclamation', 'number')}, ['!'] ),
      h('div', { className: x('challenge')}, [slide.text] ),
      h('div', { className: x('buttons')}, [
        h('div', { className: x('button') }, [
          // h('img', { src: chathelp, className: x('button-icon')}),
          h('div', { className: x('button-text')}, ['Čet pomoć'] )
        ]),
        h('div', { className: x('button') }, [
          // h('img', { src: playagain, className: x('button-icon')}),
          h('div', { className: x('button-text')}, ['Ponovite lekciju'] )
        ]),
        h('div', { className: x('button') }, [
          // h('img', { src: finish, className: x('button-icon')}),
