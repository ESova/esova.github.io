...
function LessonComponent (db, engine) {
  ...
  var STATUS = {
    ...
    slides: [],
    answers: [],
    chaturl: 'https://gitter.im/ESova/program/'
  }
      ...
      h('div', { className: x('video-container') }, [
        ...
        // @TODO: put video component into normal vcs component format
          ...
          video.bind("end", function() {
            var tasks = STATUS.slides
              .map(x=>x.type)
              .filter(x=>x=='challenge'||x=='question').length
  ...
  function evaluate (event) {
    ...
    var isCorrect = STATUS.answers.reduce(check, true)
    function check (isCorrect, selectedAnswer) {
      var x = correctAnswers.indexOf(selectedAnswer) !== -1
      return isCorrect && x
    }
    STATUS.answers = []
    if (isCorrect) {
  ...
  function renderBravo (slide, lesson, render, next) {
    ...
    if (lesson === '0' && slide.type === 'challenge') {
