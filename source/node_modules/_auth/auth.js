...
function GithubAuth (db, cb) {
  ...
  db.get('esova-token', function (error, oldToken) {
    error ? auth(settings, function start (err, newToken) {
      if (err) cb(err)
      else db.put('esova-token', newToken, function (e) {
        if (e) cb(new Error('could not store token' + newToken))
        else cb(null, newToken)
      })
    }) : cb(null, oldToken)
  })
  function auth (x, cb) {
